...
export default function CollectionTracker() {
  ...
  const usageText = collection ? `${collection.count} / ${collection.limit} Specimens Used` : 'Loading collection usage...'
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await createItem({
        ...
        dateFound: dateFound ? new Date(`${dateFound}T00:00:00`).getTime() : undefined,
  ...
  return (
    <section className="workspace-page">
      <SectionHeader
        ...
        action={<Badge tone={isAtLimit ? 'achievement' : 'neutral'}>{usageText}</Badge>}
      />

      <Card>
        <form className="collection-form" onSubmit={handleSubmit}>
          {isAtLimit ? <p className="empty-state">You've reached the free collection limit.</p> : null}
          <div className="form-grid">
            <label>
              Specimen name
              <input value={specimenName} onChange={(event) => setSpecimenName(event.target.value)} placeholder="Blue agate" required />
            </label>
            <label>
              Material type
              <input value={materialType} onChange={(event) => setMaterialType(event.target.value)} placeholder="Agate, jasper, quartz..." />
            </label>
            <label>
              Found location
              <input value={foundLocation} onChange={(event) => setFoundLocation(event.target.value)} placeholder="Central Washington" />
            </label>
            <label>
              Date found
              <input type="date" value={dateFound} onChange={(event) => setDateFound(event.target.value)} />
            </label>
            <label>
              Status
          ...
          <div className="form-footer">
            <span>{message}</span>
            <button type="submit" disabled={isSaving || isAtLimit || !specimenName.trim() || !photo}>
      ...
      {collection === undefined ? <EmptyState title="Loading collection..." /> : null}
      ...
      {collection?.items.length ? (
        <div className="collection-grid">
          {collection.items.map((item) => (
            <Link key={item._id} to={`/collections/${item._id}`} className="collection-card-link">
              <Card as="article" className="collection-card">
                <img src={item.photoUrl} alt="" />
                <div>
                  <h2>{item.specimenName}</h2>
                  <p>{item.materialType || item.foundLocation || 'Specimen details coming soon'}</p>
